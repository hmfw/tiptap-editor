...
export default defineComponent({
  ...
  setup(props) {
    ...
    return () => (
      <NodeViewWrapper class="tiptap-image-upload">
        <div onClick={handleClick}>
          {!fileItems.value.length ? (
            <div
              class={['tiptap-image-upload-drag-area', { 'drag-active': isDragActive.value }]}
            ...
            >
              <div class="tiptap-image-upload-content">
                ...
                <span class="tiptap-image-upload-subtext">
                  最多 {limit.value} 个文件
                  {maxSize.value ? `，每个不超过 ${maxSize.value / 1024 / 1024}MB` : ''}
                </span>
              </div>
            </div>
          ) : (
            <div class="tiptap-image-upload-previews">
              {fileItems.value.map((item) => (
                <div key={item.id} class="tiptap-image-upload-preview">
